		 ...
		 if(isset($listCatalogue) && is_array($listCatalogue) && count($listCatalogue)){ 
			 ...
			 foreach($listCatalogue as $key => $val){ $href = rewrite_url($val['canonical'], true, true);
				...
				$html = $html .'<tr class="gradeX">';
					...
					$html = $html.'<td><a class="maintitle" href="'.site_url('item/backend/item/view?cataloguesid='.$val['id'].'').'" title="">'.$val['title'].'</a>
<a href="'.$href.'" title="Lấy địa chỉ liên kết" onclick="prompt(\'Lấy địa chỉ liên kết\',\''.$val['canonical'].'.html\'); return false;"><img border="0" src="template/backend/img/link.png"></a>
					...
					$html = $html.'<td class="text-center">';
						...
						$html = $html.'<a type="button" href="'.(site_url('item/backend/catalogue/update/'.$val['id'].'')).'" class="btn btn-sm btn-primary mr5"><i class="fa fa-edit"></i></a>';

						$html = $html.'<a type="button" class="btn btn-sm btn-danger ajax-delete" data-title="Lưu ý: Khi bạn xóa danh mục, toàn bộ bài viết trong nhóm này sẽ bị xóa. Hãy chắc chắn bạn muốn thực hiện chức năng này!" data-id="'.$val['id'].'" data-module="item_catalogue"><i class="fa fa-trash"></i></a>';
