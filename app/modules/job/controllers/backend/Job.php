...
class Job extends MY_Controller {
		...
		if($this->input->post('create')){
			...
			if($this->form_validation->run($this)){
				...
				$_insert = array(
					...
					'hannophoso' => $this->input->post('hannophoso'),
	...
	public function Update($id = 0){
		...
		$detailjob = $this->Autoload_Model->_get_where(array(
			'select' => 'id, title, slug, canonical, description, meta_title, catalogueid, catalogue, tag, meta_description, image, publish, publish_time, amp,video,icon,`hannophoso`,`mucluong`,`diadiem`,`vitri`',
			'table' => 'job',
	...
	public function duplicate($id = 0){
		...
		$detailjob['title'] = str_duplicate(array('value' => $detailjob['title']));
		$detailjob['canonical'] = str_duplicate(array('value' => $detailjob['canonical'], 'field' => 'canonical'));
		$data['detailjob'] = $detailjob;
