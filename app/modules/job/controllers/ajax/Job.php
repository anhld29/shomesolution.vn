...
class Job extends MY_Controller {
	...
	public function listjob(){
		...
		if($config['total_rows'] > 0){
			...
			if($catalogueid > 0){
				...
				$listCatalogue = $this->Autoload_Model->_get_where(array(
					...
					'select' => 'id, catalogueid, catalogue,title, publish, created, order, viewed, image,ishome,highlight,isaside,isfooter,canonical, (SELECT fullname FROM user WHERE user.id = job.userid_created) as user_created, (SELECT title FROM job_catalogue WHERE job_catalogue.id = job.catalogueid) as catalogue_title',
		...
		$listIS = $this->Autoload_Model->_get_where(array(
			'select' => '*',
			'table' => 'general_is',
			'where' => array('module' => 'job', 'publish' => 1),
		), TRUE);
		$html = '';
						...
						$html = $html . '<td class="">
													<div class="switch">
														<div class="onoffswitch">
															<input type="checkbox" ' . (($val['' . $valIS['is'] . ''] == 1) ? 'checked=""' : '') . ' class="onoffswitch-checkbox publish_frontend" data-module="job" data-title="' . $valIS['is'] . '" data-id="' . $val['id'] . '" id="publish_' . $valIS['is'] . '-' . $val['id'] . '">
