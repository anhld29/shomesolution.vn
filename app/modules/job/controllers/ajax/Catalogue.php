	...
	public function listCatalogue(){
		...
		if($config['total_rows'] > 0){
			...
			$listCatalogue = $this->Autoload_Model->_get_where(array(
				...
				'select' => 'id, title, publish, created, order, level,ishome,highlight,isaside,isfooter, canonical,(SELECT fullname FROM user WHERE user.id = job_catalogue.userid_created) as user_created,ishome,highlight,isaside,isfooter',
...
<a href="'.$href.'" title="Lấy địa chỉ liên kết" onclick="prompt(\'Lấy địa chỉ liên kết\',\''.$val['canonical'].'.html\'); return false;"><img border="0" src="template/backend/img/link.png"></a>
				 ...
				 if (is_array($listIS) && count($listIS) && isset($listIS)) {
						 ...
						 $html = $html . '<td class="">
													<div class="switch">
														<div class="onoffswitch">
															<input type="checkbox" ' . (($val['' . $valIS['is'] . ''] == 1) ? 'checked=""' : '') . ' class="onoffswitch-checkbox publish_frontend" data-module="job_catalogue" data-title="' . $valIS['is'] . '" data-id="' . $val['id'] . '" id="publish_' . $valIS['is'] . '-' . $val['id'] . '">
					...
					$html = $html.'<td class="text-center">';
						...
						$html = $html.'<a type="button" class="btn btn-sm btn-danger ajax-delete" data-title="Lưu ý: Khi bạn xóa danh mục, toàn bộ bài viết trong nhóm này sẽ bị xóa. Hãy chắc chắn bạn muốn thực hiện chức năng này!" data-id="'.$val['id'].'" data-module="job_catalogue"><i class="fa fa-trash"></i></a>';
