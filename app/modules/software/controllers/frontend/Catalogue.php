...
class Catalogue extends MY_Controller {
	...
	public function View($id = 0, $page = 1){
		...
		if($config['total_rows'] > 0){
			...
			$config['full_tag_open'] = '<div class="ttm-pagination">';
			...
			$data['articleList'] = $this->Autoload_Model->_condition(array(
				...
				'select' => '`object`.`id`, `object`.`title`,`object`.`canonical`, `object`.`image`, `object`.`created`, `object`.`viewed`, `object`.`description`, (SELECT fullname FROM user WHERE user.id = object.userid_created) as user_created, (SELECT title FROM software_catalogue WHERE software_catalogue.id = object.catalogueid) as catalogue_title, (SELECT count(id) FROM comment WHERE comment.detailid = object.id AND comment.module = \'software\') as comment',
		...
		$data['module'] = 'software_catalogue';
		...
		$data['template'] = 'training/frontend/catalogue/view';
