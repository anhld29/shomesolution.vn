	...
	public function view($id = 0){
		...
		if(!isset($this->auth) || is_array($this->auth) == false || count($this->auth) == 0){
			$detailArticle = $this->Autoload_Model->_get_where(array(
				...
				'select' => 'id, title, slug, canonical, catalogueid, description, image, viewed, (SELECT fullname FROM user WHERE user.id = software.userid_created) as fullname, created',
		...
		$objectSee = isset($_COOKIE[CODE.'softwareCookie'])?$_COOKIE[CODE.'softwareCookie']:NULL;

		$objectid = json_decode($objectSee, TRUE);

		if (!isset($objectSee) || empty($objectSee)) {

			setcookie(CODE.'softwareCookie', json_encode(array(
			...
			setcookie(CODE.'softwareCookie', json_encode($objectid), time() + (86400 * 30), '/');
