...
class Catalogue extends MY_Controller {
	...
	public function view($page = 1){
		$this->commonbie->permission("software/backend/catalogue/view", $this->auth['permission']);
		$page = (int)$page;
		$data['from'] = 0;
		$data['to'] = 0;
		...
		if($config['total_rows'] > 0){
			...
			$config['base_url'] = base_url('software/backend/catalogue/view');
			...
			$data['listCatalogue'] = $this->Autoload_Model->_get_where(array(
				'select' => 'id, title, canonical, publish,ishome,highlight,isaside,isfooter, created, order, level, lft, rgt, (SELECT fullname FROM user WHERE user.id = software_catalogue.userid_created) as user_created',
				'table' => 'software_catalogue',
				'where' => array( 'alanguage' => $this->fclang),
		...
		$data['script'] = 'software_catalogue';
		$data['config'] = $config;
		$data['template'] = 'software/backend/catalogue/view';
		$this->load->view('dashboard/backend/layout/dashboard', isset($data)?$data:NULL);
	}
	
	public function Create(){
		$this->commonbie->permission("software/backend/catalogue/create", $this->auth['permission']);
				...
				if($resultid > 0){
					...
					if(!empty($canonical)){
						$router = array(
							...
							'uri' => 'software/frontend/catalogue/view',
							'param' => $resultid,
							'type' => 'number',
							'created' => gmdate('Y-m-d H:i:s', time() + 7*3600),
						);
					...
					redirect('software/backend/catalogue/create');
				}
			}
		}
		
		$data['template'] = 'software/backend/catalogue/create';
		$this->load->view('dashboard/backend/layout/dashboard', isset($data)?$data:NULL);
	}
	
	public function Update($id = 0){
		$this->commonbie->permission("software/backend/catalogue/update", $this->auth['permission']);
		$id = (int)$id;
		...
		if(!isset($detailCatalogue) || is_array($detailCatalogue) == false || count($detailCatalogue) == 0){
			...
			redirect('software/backend/catalogue/view');
		}
				...
				if($flag > 0){
					...
					if(!empty($canonical)){
						$this->Autoload_Model->_delete(array(
							'where' => array('canonical' => $detailCatalogue['canonical'],'uri' => 'software/frontend/catalogue/view','param' => $id),
							'table' => 'router',
						));
						$router = array(
							'canonical' => $canonical,
					...
					redirect('software/backend/catalogue/update/'.$id.'');
				}
			}
		}
		...
		$data['template'] = 'software/backend/catalogue/update';
		$this->load->view('dashboard/backend/layout/dashboard', isset($data)?$data:NULL);
	}
