...
class Software extends MY_Controller {
	...
	public function listsoftware(){
		...
		if(!in_array('software/backend/software/viewall', json_decode($this->auth['permission'], TRUE))){
		...
		if($config['total_rows'] > 0){
			...
			if($catalogueid > 0){
				...
				$listCatalogue = $this->Autoload_Model->_condition(array(
					...
					'select' => '`object`.`id`, `object`.`title`, `object`.`slug`,`object`.`canonical`, `object`.`image`, `object`.`catalogueid`, `object`.`catalogue`, `object`.`publish`, `object`.`created`, `object`.`order`, `object`.`viewed`,`object`.`highlight`,`object`.`ishome`, (SELECT fullname FROM user WHERE user.id = object.userid_created) as user_created, (SELECT title FROM software_catalogue WHERE software_catalogue.id = object.catalogueid) as catalogue_title',
				...
				$listCatalogue = $this->Autoload_Model->_get_where(array(
					...
					'select' => 'id, catalogueid, catalogue,title, publish, created, order, viewed, image,highlight,ishome,canonical, (SELECT fullname FROM user WHERE user.id = software.userid_created) as user_created, (SELECT title FROM software_catalogue WHERE software_catalogue.id = software.catalogueid) as catalogue_title',
					...
					$html = $html.'<td>';
						...
						$html = $html.'<div class="uk-flex uk-flex-middle">';
							...
							$html = $html.'<div class="main-info">';
								...
								$html = $html.'<div class="title"><a class="maintitle" href="javascript:void(0)" title="">'.$val['title'].'</a>
<a href="'.$href.'" title="Lấy địa chỉ liên kết" onclick="prompt(\'Lấy địa chỉ liên kết\',\''.$val['canonical'].'.html\'); return false;"><img border="0" src="template/backend/img/link.png"></a>
								...
								$html = $html.'<div class="catalogue" style="font-size:10px">';
									...
									$html = $html.'<a class="" style="color:#333;" href="'.site_url('software/backend/software/view?catalogueid='.$val['catalogueid']).'" title="">'.$val['catalogue_title'].'</a>'.((isset($_catalogue_list) && is_array($_catalogue_list) && count($_catalogue_list) ) ? ' ,' :'').'';

									if(isset($_catalogue_list) && is_array($_catalogue_list) && count($_catalogue_list)){ foreach($_catalogue_list as $keyCat => $valCat){ 

										$html = $html.'<a style="color:#333;" class="" href="'.site_url('software/backend/software/view?catalogueid='.$valCat['id']).'" title="">'.$valCat['title'].'</a> '.(($keyCat + 1 < count($_catalogue_list)) ? ', ' : '').'';
				...
				$html = $html.'<input type="checkbox" '.(($val['ishome'] == 1) ? 'checked=""' : '').' class="onoffswitch-checkbox publish_frontend" data-module="software" data-title="ishome" data-id="'.$val['id'].'" id="publish_ishome'.$val['id'].'">';
				...
				$html = $html.'<input type="checkbox" '.(($val['highlight'] == 1) ? 'checked=""' : '').' class="onoffswitch-checkbox publish_frontend" data-module="software" data-title="highlight" data-id="'.$val['id'].'" id="publish_highlight'.$val['id'].'">';
				...
				$html = $html.'</td>';
					...
					$html = $html.'<td class="text-center">';
						...
						$html = $html.'<a title="Thêm" type="button" href="'.(site_url('software/backend/software/update/'.$val['id'].'')).'" class="btn btn-sm btn-primary mr5"><i class="fa fa-edit"></i></a>';

						$html = $html.'<a title="Sửa" type="button" class="btn btn-sm btn-danger ajax-delete" data-title="Lưu ý: Khi bạn xóa danh mục, toàn bộ bài viết trong nhóm này sẽ bị xóa. Hãy chắc chắn bạn muốn thực hiện chức năng này!" data-id="'.$val['id'].'" data-module="software"><i class="fa fa-trash"></i></a><a title="Sao chép" type="button" href="'.site_url('software/backend/software/duplicate/'.$val['id'].'?page=1').'" class="btn btn-info" title="Sao chép"><i class="fa fa-files-o" aria-hidden="true"></i></a>
													<a title="Xem trước" target="_blank" href="'.$val['canonical'].'.html" class="btn btn-info" title="Xem trước"><i class="fa fa-eye" aria-hidden="true"></i></a>';
